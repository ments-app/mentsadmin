...
import { getStartupEvents, deleteStartupEvent } from '@/actions/startup-portal';
...
const columns: Column<Event>[] = [
  {
    ...
    render: (item) => (
      <span className="flex items-center gap-1.5">
        {item.is_featured && <Star size={13} className="shrink-0 fill-amber-400 text-amber-400" />}
        {item.title}
      </span>
    ),
  },
  {
    ...
    label: 'Category',
    render: (item) => (
      <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium capitalize ${CATEGORY_STYLES[item.category] ?? 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}`}>
        {item.category}
      </span>
    ),
  },
...
export default function StartupEventsPage() {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await deleteStartupEvent(deleteTarget.id);
