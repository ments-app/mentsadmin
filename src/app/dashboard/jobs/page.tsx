...
const columns: Column<Job & { _appCount?: number }>[] = [
  ...
  },
  {
    key: 'company',
    label: 'Company',
    render: (item) => <span className="text-sm text-muted">{item.company}</span>,
  },
  {
    key: 'job_type',
    label: 'Type',
    render: (item) => (
      <span className="inline-block rounded-full bg-blue-100 px-2.5 py-1 text-xs font-medium capitalize text-blue-700 dark:bg-blue-900 dark:text-blue-300">
        {item.job_type}
      </span>
    ),
  },
  ...
  },
  {
    ...
    render: (item) =>
      item.visibility === 'facilitator_only' ? (
        <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2.5 py-1 text-xs font-medium text-amber-700 dark:bg-amber-900 dark:text-amber-300">
          <Lock size={10} />
          Facilitators
        </span>
      ) : (
  ...
  },
  {
    ...
    render: (item) => {
      ...
      return count > 0 ? (
        <Link href={`/dashboard/jobs/${item.id}/applications`} className="inline-flex items-center gap-1 rounded-full bg-purple-100 px-2.5 py-1 text-xs font-semibold text-purple-700 dark:bg-purple-900 dark:text-purple-300 hover:bg-purple-200 dark:hover:bg-purple-800 transition-colors">
          {count}
        </Link>
      ) : (
  ...
  },
  {
    key: 'created_at',
    label: 'Created',
    render: (item) => <span className="text-sm text-muted">{format(new Date(item.created_at), 'MMM d, yyyy')}</span>,
  },
];
