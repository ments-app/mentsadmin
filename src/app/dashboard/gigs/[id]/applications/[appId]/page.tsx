...
export default function GigApplicantDetailPage() {
  ...
  if (!app) return <div className="text-center py-20 text-muted">Application not found</div>;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="rounded-lg border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/10 p-4">
            ...
            <ul className="space-y-1">
              {app.strengths.map((s, i) => (
                <li key={i} className="text-xs text-foreground flex items-start gap-1.5"><span className="text-emerald-500 mt-0.5">+</span> {s}</li>
              ))}
            </ul>
          </div>
        )}
        {app.weaknesses.length > 0 && (
          <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/10 p-4">
            <h3 className="text-sm font-semibold text-amber-700 dark:text-amber-300 mb-2">Gaps</h3>
            <ul className="space-y-1">
              {app.weaknesses.map((w, i) => (
                <li key={i} className="text-xs text-foreground flex items-start gap-1.5"><span className="text-amber-500 mt-0.5">-</span> {w}</li>
              ))}
            </ul>
          </div>
        )}
      ...
      {/* Q&A */}
      <div className="rounded-lg border border-card-border bg-background p-5 mb-4">
        <h2 className="text-sm font-semibold text-foreground mb-4">Interview Q&A</h2>
            ...
            return (
              <div key={q.id} className="border border-card-border rounded-lg p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div><span className="text-xs font-semibold text-primary bg-primary/10 px-2 py-0.5 rounded mr-2">Q{i + 1}</span><span className="text-xs text-muted uppercase">{q.type.replace('_', ' ')}</span></div>
                ...
                <div className="bg-card-border/10 rounded-lg p-3 mb-2"><p className="text-sm text-foreground">{q.answer || <em className="text-muted">No answer</em>}</p></div>
      ...
      {(app.ai_summary || app.hire_suggestion) && (
        <div className="rounded-lg border border-card-border bg-background p-5 mb-4">
          ...
          {app.ai_summary && <div className="mb-3"><h3 className="text-xs font-semibold text-muted mb-1">Summary</h3><p className="text-sm text-foreground leading-relaxed">{app.ai_summary}</p></div>}
          {app.hire_suggestion && <div><h3 className="text-xs font-semibold text-muted mb-1">Hire Suggestion</h3><p className="text-sm text-foreground leading-relaxed">{app.hire_suggestion}</p></div>}
        </div>
      )}
          ...
          <div className="flex flex-wrap gap-2">
            {['submitted', 'reviewed', 'shortlisted', 'rejected'].map((s) => (
              <button key={s} onClick={() => handleStatusChange(s)} disabled={saving || app.status === s}
                className={`px-4 py-2 rounded-lg text-xs font-medium transition-colors border ${app.status === s ? 'bg-primary text-white border-primary' : 'border-card-border text-foreground hover:bg-card-border/30'} disabled:opacity-50`}>
