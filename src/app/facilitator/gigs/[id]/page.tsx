...
export default function FacilitatorEditGigPage() {
  ...
  const [form, setForm] = useState({
    ...
    skills_required: [] as string[], deadline: '', is_active: true,
    company: '', company_logo_url: '', company_website: '',
    category: 'other', experience_level: 'any', payment_type: 'fixed',
    deliverables: '', responsibilities: '', contact_email: '',
  ...
  useEffect(() => {
    getGig(id).then((data) => {
      setForm({
        ...
        budget: data.budget ?? '', duration: data.duration ?? '',
        skills_required: data.skills_required ?? [],
        deadline: data.deadline ? data.deadline.slice(0, 16) : '',
        is_active: data.is_active, company: data.company ?? '',
        company_logo_url: data.company_logo_url ?? '',
        company_website: data.company_website ?? '',
        category: data.category ?? 'other', experience_level: data.experience_level ?? 'any',
        payment_type: data.payment_type ?? 'fixed',
        deliverables: data.deliverables ?? '', responsibilities: data.responsibilities ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try { await updateGig(id, form); router.push('/facilitator/gigs'); }
  ...
  async function handleDelete() {
    setDeleting(true);
    try { await deleteGig(id); router.push('/facilitator/gigs'); }
    ...
    return (
      <div className="mx-auto max-w-2xl animate-fade-in">
        ...
        <div className="mt-6 space-y-4">
          {Array.from({ length: 5 }).map((_, i) => (<div key={i} className="h-12 skeleton-shimmer rounded-xl" />))}
        </div>
      </div>
    );
  }
        ...
        <div className="card-elevated p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input type="url" value={form.company_logo_url} onChange={(e) => update('company_logo_url', e.target.value)} placeholder="https://example.com/logo.png" className="flex-1 rounded-xl border border-card-border bg-background px-3.5 py-2.5 text-sm text-foreground outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors" />
